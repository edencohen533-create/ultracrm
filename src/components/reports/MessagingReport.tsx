...
export function MessagingReport({ days, stats, basePath = "/analytics" }: { days: number; stats: MessagingStats; basePath?: string }) {
  return (
    <div className="space-y-6 p-6">
      <div>
        <h1 className="text-lg font-semibold">אנליטיקה</h1>
        <div className="mb-2 flex gap-2 text-sm">{[7, 30, 90].map((d) => <a key={d} href={`${basePath}${basePath.includes("?") ? "&" : "?"}days=${d}`} className={`rounded-full border px-3 py-1 ${days === d ? "bg-primary text-primary-foreground" : ""}`}>{d} ימים</a>)}</div>
