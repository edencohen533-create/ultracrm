...
export function CampaignDashboard({ initialCampaigns, lists, contacts, templates, channelTemplates, channels, timezone, marketingWindow, mock, senders = [], audienceOptions = { tags: [], agents: [], campaigns: [] }, mode = "all", fixedChannel }: Props) {
  ...
  return <div className="mx-auto max-w-6xl space-y-6 p-6" dir="rtl">
    {mode !== "all" && <nav className="campaigns-nav" aria-label="קמפיינים" data-testid="campaigns-nav">
      ...
      <div className="campaigns-secondary"><a href="/audiences" className={mode === "audiences" ? "active" : ""} data-testid="campaigns-audiences">קהלים ואנשי קשר</a><a href="/templates" data-testid="campaigns-templates">תבניות</a></div>
    </nav>}
