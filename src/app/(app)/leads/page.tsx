...
export default function LeadsPage() {
  ...
  const [filter, setFilter] = useState({ status: "", q: "", ownerUserId: "" });
  ...
  const load = useCallback(() => api.get<typeof data>(`/api/leads${qs({ ...filter, page, limit: 30 })}`).then(setData).catch((e) => toast.error(e.message)), [filter, page]);
  ...
  return (
    <div className="p-5 space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Button size="sm" className="ms-auto" onClick={() => setOpen(true)}>+ ליד</Button>
      </div>
      ...
      {!data ? <div className="flex justify-center p-10"><Spinner /></div> : data.items.length === 0 ? <EmptyState title="אין לידים" hint="צור ליד מאיש קשר; שיוך לנציג ומשימת פנייה ראשונית נוצרים אוטומטית" /> : (
        <div className="bg-panel border border-line rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead className="text-xs text-muted bg-white/3"><tr><th className="text-start px-3 h-9 font-medium">איש קשר</th><th className="text-start px-3 font-medium">ליד</th><th className="text-start px-3 font-medium">מקור</th><th className="text-start px-3 font-medium">נציג</th><th className="text-start px-3 font-medium">סטטוס</th><th className="text-start px-3 font-medium">נוצר</th><th className="px-3"></th></tr></thead>
            <tbody className="divide-y divide-line">
              {data.items.map((l) => (
                <tr key={l.id} className="hover:bg-white/3">
                  <td className="px-3 h-11"><Link href={`/contacts/${l.contact.id}`} className="font-medium hover:underline">{l.contact.fullName}</Link><span className="block text-[11px]"><Phone value={formatPhone(l.contact.phoneE164)} className="text-muted" /></span></td>
                  <td className="px-3"><Link href={`/leads/${l.id}`} className="hover:underline">{l.title ?? "—"}</Link></td>
                  <td className="px-3 text-muted">{l.source ?? "—"}</td>
                  <td className="px-3 text-muted">{l.owner?.fullName ?? "ללא"}</td>
                  <td className="px-3"><Select value={l.status} onChange={(e) => setStatus(l.id, e.target.value)} className="h-8 text-xs w-36">{LEAD_STATUSES.map((s) => <option key={s} value={s}>{LEAD_STATUS_LABEL[s]}</option>)}</Select></td>
                  <td className="px-3 text-xs text-muted tabular">{relativeTime(l.createdAt)}</td>
                  <td className="px-3 text-end whitespace-nowrap">
                    {me?.modules.telephony && <Button size="sm" variant="good" disabled={!canDial} onClick={() => dial({ mode: "manual", contactId: l.contact.id })}>חייג</Button>}
