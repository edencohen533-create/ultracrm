...
export function CampaignReport({ id }: { id: string }) {
  ...
  const total = Object.values(report.recipients).reduce((a, b) => a + b, 0);
  const sent = report.recipients.SENT ?? 0; const failed = (report.recipients.FAILED ?? 0) + (report.recipients.UNKNOWN ?? 0); const skipped = report.recipients.SKIPPED ?? 0;
    ...
    <div className="rep" data-testid="campaign-report">
      ...
      {tab === "overview" && <div className="rep-body">
        <section className="rep-card"><h2>סיכום</h2><div className="rep-metrics">
          ...
          {report.channel === "email" && metric("הקלקות ייחודיות", report.availability.clicks === "signal" ? report.engagement.clicked : null, report.availability.clicks === "signal" ? pct(report.engagement.clicked, sent) : "", report.availability.clicks === "signal" ? "" : "אין מעקב הקלקות")}
          {report.channel !== "email" && metric("נקראו", report.delivery.READ ?? null, report.delivery.READ != null ? pct(report.delivery.READ, sent) : "", report.availability.delivery === "unavailable" ? "אין דיווח מהספק" : undefined)}
      ...
      {tab === "links" && <div className="rep-body"><section className="rep-card"><h2>קישורים בתוכן</h2>{links?.links.length ? <ul className="rep-list">{links.links.map((l) => <li key={l.url}><a href={l.url} target="_blank" rel="noreferrer" dir="ltr">{l.url}</a><strong>{l.uniqueClicks ?? "—"}</strong></li>)}</ul> : <p className="rep-empty">אין קישורים בתוכן</p>}<p className="rep-note">{links?.note}{links?.totalUniqueClicks !== null && links?.totalUniqueClicks !== undefined ? ` סה״כ הקלקות ייחודיות בקמפיין: ${links.totalUniqueClicks}.` : ""}</p></section></div>}
    </div>
  );
}
