...
function TaskEditor({ task, assignees, checkedAt, saved }: { task: Task; assignees: Assignee[]; checkedAt: number; saved: () => Promise<void> }) {
  ...
  return <details className="rounded border p-2">
    <summary className="cursor-pointer break-words text-sm"><span dir="auto">{task.title ?? "משימה"}</span> · {labels[task.status]} · {new Date(task.dueAt).toLocaleString("he-IL")} · {task.user.fullName}{task.status === "open" && Date.parse(task.dueAt) < checkedAt ? " · באיחור" : ""}</summary>
    <form className="mt-2 grid gap-2" onSubmit={submit}>
      ...
      <label className="text-sm">נציג<select className="block w-full rounded border p-2 bg-bg" aria-label={`נציג למשימה ${task.id}`} value={userId} onChange={(e) => setUserId(e.target.value)}>{!assignees.some((a) => a.id === task.userId) && <option value={task.userId}>{task.user.fullName} (אינו זמין)</option>}{assignees.map((a) => <option key={a.id} value={a.id}>{a.fullName}</option>)}</select></label>
      <label className="text-sm">סטטוס<select className="block w-full rounded border p-2 bg-bg" aria-label={`סטטוס משימה ${task.id}`} value={status} onChange={(e) => setStatus(e.target.value)}>{Object.entries(labels).map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></label>
...
export function Tasks({ contactId, conversationId, userId }: { contactId: string; conversationId?: string; userId: string }) {
  ...
  return <details className="border-b px-3 py-2" onToggle={(event) => { if (event.currentTarget.open && !loaded && !loading) void load(); }}>
    ...
    <div className="mt-2 max-h-[55dvh] space-y-3 overflow-y-auto p-1">
      <p className="text-xs text-muted-foreground">משימות פנימיות בלבד; אינן שולחות הודעה ללקוח. מופיעות גם בכרטיס הלקוח ובמסך המשימות.</p>
      {error && <p role="alert" className="text-sm text-destructive">{error}</p>}
      ...
      {loaded && <>
        <form onSubmit={create} className="grid gap-2 rounded border p-3">
          ...
          <label className="text-sm">נציג אחראי<select aria-label="נציג למשימה חדשה" className="block w-full rounded border p-2 bg-bg" value={assignee} onChange={(e) => setAssignee(e.target.value)} required>{assignees.map((a) => <option key={a.id} value={a.id}>{a.fullName}</option>)}</select></label>
