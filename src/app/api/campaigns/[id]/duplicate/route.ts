...
export const POST = organizationRequest(async function(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const campaign = await createCampaign({ name: `${source.name.slice(0, 110)} — העתק`, listId: source.listId, excludedListIds: Array.isArray(source.excludedListIds) ? source.excludedListIds.filter((id): id is string => typeof id === "string") : [], providerCredentialId: source.providerCredentialId, templateId: source.templateId, variables: source.variables as Record<string, string> }, actor.id);
