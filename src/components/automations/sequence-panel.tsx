...
export interface SequenceRow { id: string; name: string; isActive: boolean; trigger: string; triggerConfig: { channel?: string; tagName?: string; marketingOnly?: boolean; leadStatus?: string; contactSource?: string }; stopOn: string[]; steps: Array<{ position: number; action?: string; channel: string; templateId: string | null; waitMinutes: number; variables?: Record<string, string>; condition?: Step["condition"]; template: { name: string } | null }>; _count: { runs: number } }
      ...
      {sequences.length === 0 ? <p className="text-sm text-muted-foreground">אין רצפים עדיין.</p> : (
        <ul className="space-y-2">{sequences.map((s) => (
          <li key={s.id} className="rounded-xl border p-3 text-sm" data-testid={`sequence-${s.id}`}>
            ...
            <ol className="mt-1 flex flex-wrap gap-2 text-xs text-muted-foreground">{s.steps.map((st) => <li key={st.position} className="rounded bg-muted px-2 py-0.5">{st.position + 1}. המתנה {st.waitMinutes} דק׳ → {st.action === "task" ? `משימה: ${st.variables?.__taskTitle ?? ""}` : `${CHANNEL_LABELS[st.channel]}: ${st.template?.name ?? "תבנית חסרה"}`}</li>)}</ol>
          </li>
        ))}</ul>
      )}
    </section>
