  ...
  return (
    <div className="p-5 space-y-4">
      ...
      <div className="flex gap-1 border-b border-line" role="tablist">
        <button role="tab" aria-selected={view === "leads"} onClick={() => setView("leads")} className={`h-9 px-3 text-sm border-b-2 -mb-px ${view === "leads" ? "border-accent font-medium" : "border-transparent text-muted"}`} data-testid="list-view-leads">לידים וחייגן</button>
        <button role="tab" aria-selected={view === "queue"} onClick={() => setView("queue")} className={`h-9 px-3 text-sm border-b-2 -mb-px ${view === "queue" ? "border-accent font-medium" : "border-transparent text-muted"}`} data-testid="list-view-queue">תור החיוג ({total})</button>
      </div>
