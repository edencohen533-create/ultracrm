...
import { ArrowDownUp, CalendarDays, Check, CheckSquare, ChevronLeft, ChevronRight, Download, Filter, Maximize2, MessageCircle, Minimize2, Percent, Phone as PhoneIcon, RefreshCw, Search, Settings, ShoppingBag, TrendingUp, Users, X } from "lucide-react";
...
/**
 * The lead workspace: list + metrics + dialer dock + lead drawer + tasks drawer + dialer settings.
...
export function LeadsWorkspace({ listId, listName, listHeader }: { listId?: string; listName?: string; listHeader?: React.ReactNode } = {}) {
  ...
  const [filter, setFilter] = useState({ status: params.get("status") ?? "", q: params.get("q") ?? "", ownerUserId: params.get("mine") === "1" ? "me" : params.get("ownerUserId") ?? "", source: "", product: "", campaign: "", ad: "", period: listId ? "all" : "month", from: "", to: "" });
  ...
  const load = useCallback(async () => {
    ...
    try { const result = await api.get<LeadData>(`/api/leads${qs({ ...filter, ownerUserId: owner, ...dates, listId, sort: sort.key, direction: sort.direction, page, limit: 30 })}`); if (id === requestId.current) setData(result); }
  ...
  useEffect(() => { if (params.get("tasks") === "1") setTasksOpen(true); if (params.get("settings") === "1") setSettingsOpen(true); }, [params]);
  ...
  return <div className="leads-page" data-testid="leads-redesign">
    ...
    <div className="leads-tools"><span>טווח: {periods[filter.period]}</span><div>{telephony && <><button className="lead-button dialer-launch" data-testid="open-dialer" disabled={!state} onClick={() => { setDock(true); setMinimized(false); }}><PhoneIcon size={18}/>הפעל חייגן</button><select aria-label="מיקום החייגן" value={side} onChange={e => moveDock(e.target.value as "left" | "right")}><option value="left">שמאל למעלה</option><option value="right">ימין למעלה</option></select></>}</div></div>
    ...
    <section className="lead-table-card" aria-busy={loading}>
      {!data ? <div className="p-12 flex justify-center"><Spinner/></div> : <><div className="lead-table-scroll"><table className="leads-table"><thead><tr><th><input type="checkbox" aria-label="בחר את כל הלידים בעמוד" checked={data.items.length > 0 && data.items.every(l => selected.includes(l.id))} onChange={e => setSelected(e.target.checked ? data.items.map(l => l.id) : [])}/></th><th><button onClick={() => sorting("name")}>שם <ArrowDownUp size={13}/></button></th><th>טלפון</th><th><button onClick={() => sorting("source")}>מקור <ArrowDownUp size={13}/></button></th><th>מוצר</th><th>קמפיין</th><th>מודעה</th><th><button onClick={() => sorting("status")}>סטטוס <ArrowDownUp size={13}/></button></th><th><button onClick={() => sorting("owner")}>נציג <ArrowDownUp size={13}/></button></th><th><button onClick={() => sorting("createdAt")}>נוצר <ArrowDownUp size={13}/></button></th><th>פעולות</th></tr></thead><tbody>{groups.map(([name, rows]) => <Fragment key={name}>{group && <tr className="lead-group"><td colSpan={11}>{name} · {rows.length} בעמוד זה</td></tr>}{rows.map(l => <tr key={l.id} data-testid={`lead-row-${l.id}`} className={selected.includes(l.id) ? "selected" : ""}>
        ...
        <td><select aria-label={`סטטוס ${l.contact.fullName}`} className={`lead-status status-${l.status}`} value={l.status} onChange={e => patch(l.id, { status: e.target.value })}>{statuses.items.filter(s => !s.hidden || s.key === l.status).map(s => <option key={s.key} value={s.key}>{s.label}</option>)}</select></td>
    ...
    {telephony && (dock || live) && <section className={`lead-dialer-dock dock-${side} ${minimized ? "minimized" : ""} ${expanded ? "expanded" : ""}`} aria-label="חייגן" data-testid={dock ? "dialer-embedded" : "dialer-hidden"} hidden={!dock}><header><strong><PhoneIcon size={17}/> {live ? "חייגן פעיל" : "הפעלת חייגן"}</strong><div><button title="העבר לצד השני" aria-label="העבר חייגן לצד השני" onClick={() => moveDock(side === "left" ? "right" : "left")}><ArrowDownUp className="rotate-90" size={16}/></button><button title="הרחב" aria-label="הרחב חייגן" onClick={() => { setExpanded(v => !v); setMinimized(false); }}><Maximize2 size={16}/></button>{live && <button title="מזער (השיחה תמשיך)" aria-label="מזער חייגן" onClick={() => setMinimized(v => !v)}><Minimize2 size={16}/></button>}<button title={live ? "סגור את החלון (השיחה ממשיכה ברקע)" : "סגור"} aria-label="סגור חייגן" data-testid="dialer-close" onClick={() => setDock(false)}><X size={17}/></button></div></header><div className="lead-dialer-body" hidden={minimized}>{live ? <DialerWorkspace embedded compact={!expanded}/> : <div className="p-4"><StartSessionForm compact onStarted={() => { setDock(true); void load(); }}/></div>}</div></section>}
    {telephony && live && !dock && <button className="dialer-live-pill" onClick={() => { setDock(true); setMinimized(false); }} data-testid="dialer-reopen">📞 החייגן פעיל – פתח</button>}
    ...
    {tasksOpen && <aside className="lead-side-drawer" aria-label="משימות וחזרות" data-testid="tasks-drawer"><header><strong>משימות וחזרות</strong><button aria-label="סגור משימות" onClick={() => { setTasksOpen(false); if (params.get("tasks")) router.replace(listId ? `/lists/${listId}` : "/leads"); }}><X size={17}/></button></header><div><TasksPanel embedded /></div></aside>}
    <LeadsSettingsModal open={settingsOpen} onClose={closeSettings} manager={manager} />
